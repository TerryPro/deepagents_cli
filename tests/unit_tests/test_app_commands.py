...
class TestSkillsCommand:
    async def test_skills_command_no_skills(self):
        ...
        with (
            ...
            patch("deepagents_cli.config.Settings.get_user_skills_dir", return_value="user_dir"),
            patch(
                "deepagents_cli.config.Settings.get_project_skills_dir", return_value="project_dir"
            ),
        ):
    ...
    async def test_skills_command_with_skills(self):
        ...
        with (
            patch("deepagents_cli.skills.load.list_skills", return_value=mock_skills),
            patch("deepagents_cli.config.Settings.get_user_skills_dir", return_value="user_dir"),
            patch(
                "deepagents_cli.config.Settings.get_project_skills_dir", return_value="project_dir"
            ),
