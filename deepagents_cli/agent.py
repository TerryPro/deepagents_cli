...
- Use virtual paths starting with `/` for all file operations
- Paths are rooted at the current working directory shown above
- Example: To read `pyproject.toml` in the current directory, use `/pyproject.toml`
- Never use Windows drive-letter paths (e.g., `C:\\...` or `F:\\...`) with file tools
...
def create_cli_agent(
    ...
    *,
    ...
    """
    tools = tools or []
    composite_backend: CompositeBackend | None = None
    ...
    skills_routes: dict[str, FilesystemBackend] = {}
    if enable_skills and skills_dir is not None:
        skills_routes["/skills/user/"] = FilesystemBackend(root_dir=skills_dir, virtual_mode=True)
        if project_skills_dir is not None:
            skills_routes["/skills/project/"] = FilesystemBackend(
                root_dir=project_skills_dir, virtual_mode=True
            )
    ...
    if enable_skills:
        ...
        agent_middleware.append(
            SkillsMiddleware(
                backend=lambda _rt: composite_backend,
                sources=sources,
            )
        )
